...
const Auth: React.FC = () => {
  ...
  const login = async () => {
    try {
      ...
      if (res.status === 200) {
        const options = { path: '/' } //index以下でcookieが使用できる
        cookie.set('access_token', res.data.access, options) //res.data.accessにJWT tokenが保存されている
        router.push('/')
      }
    } catch {
  ...
  const authUser = async (e: React.FormEvent<HTMLFormElement>) => { //onSubmitのデータ型
    e.preventDefault()
    if (isLogin) {
      login()
    } else {
